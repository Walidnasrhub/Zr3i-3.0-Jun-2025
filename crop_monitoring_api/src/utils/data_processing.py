...
def calculate_ndvi(red_band_path, nir_band_path):
    ...
    with rasterio.open(red_band_path) as red_src:
        with rasterio.open(nir_band_path) as nir_src:
            ...
            return xr.DataArray(ndvi, coords={'y': range(ndvi.shape[0]), 'x': range(ndvi.shape[1])})
    ...
    with rasterio.open(blue_band_path) as blue_src:
        with rasterio.open(red_band_path) as red_src:
            with rasterio.open(nir_band_path) as nir_src:
                ...
                return xr.DataArray(evi, coords={'y': range(evi.shape[0]), 'x': range(evi.shape[1])})
...
def apply_cloud_mask(image_path, s2_scl_path):
    ...
    with rasterio.open(image_path) as src:
        with rasterio.open(s2_scl_path) as scl_src:
            ...
            return xr.DataArray(masked_image, coords={'y': range(masked_image.shape[0]), 'x': range(masked_image.shape[1])})
